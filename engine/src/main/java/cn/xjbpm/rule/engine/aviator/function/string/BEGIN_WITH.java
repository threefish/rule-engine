...
/**
 * 字符串开头匹配函数
 * 强化了防御性编程，判断字符串是否以指定前缀开头，严格执行参数非空校验。
 *
...
public class BEGIN_WITH extends AbstractBaseFunction {
    ...
    public AviatorObject call(Map<String, Object> env, AviatorObject arg1, AviatorObject arg2) {
        ...
        Assert.notNull(value2, String.format("函数 %s 的匹配前缀(var2)不能为空", getName()));

        String var1 = value1.toString();
        String var2 = value2.toString();

        // 如果前缀为空字符串，按照标准逻辑返回 true
        if (var2.isEmpty()) {
            return AviatorBoolean.TRUE;
        }

        return AviatorBoolean.valueOf(var1.startsWith(var2));
    }
    ...
    public List<AviatorExtendFunction> docs() {
        return Collections.singletonList(
                new AviatorExtendFunction(
                        ...
                        "boolean",
                        "判断字符串 var1 是否以 var2 开头。任意参数为 null 将抛出异常。",
