...
public class DownloadFileNodeBehavior implements NodeBehavior {
    ...
    public void execution(FlowContext context) throws Exception {
        ...
        HttpCallResult httpCallResult = HttpCallUtil.executeStreaming(node.getMethod(), url, body, headers, queryParams, TimeoutOptions.VERAY_SLOW, (inputStream, clientHttpResponse) -> {
            FileUtil.writeFromStream(inputStream, file);
        });
        if (context.isDebugModel()) {
            context.addTraceLog(StringUtils.format("[{}] http call params:'{}' header:'{}' body:'{}' result:'{}'", node.getId(), JsonUtils.obj2Json(queryParams), JsonUtils.obj2Json(headers), body, JsonUtils.obj2Json(httpCallResult)));
        }
