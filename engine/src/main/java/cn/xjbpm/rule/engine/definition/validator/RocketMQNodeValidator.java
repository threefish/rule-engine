 ...
 */
public class RocketMQNodeValidator extends BaseNodeValidator<RocketMQNode> {

    @Override
    public void check(RocketMQNode node) {
        super.check(node);
        Assert.isTrue(StringUtils.isNotBlank(node.getCredentialId()), 
            "RocketMQ凭据不能为空");
        Assert.isTrue(StringUtils.isNotBlank(node.getTopic()), 
            "Topic不能为空");
        Assert.isTrue(StringUtils.isNotBlank(node.getMessageBody()), 
            "消息内容不能为空");
    }
}
